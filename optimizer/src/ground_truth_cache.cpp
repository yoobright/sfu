...
std::vector<uint32_t> GroundTruthCache::generate_inputs(SFUOp op,
  ...
  if (op == SFUOp::TANH) {
    // Exhaust the shared TANH table exactly once per local coordinate.
    ...
    for (uint32_t segment = 0; segment < kSegments; ++segment) {
      float start = segment < 64
                        ? static_cast<float>(segment) * 0x1p-6f
                        : segment < 112
                              ? 1.0f + static_cast<float>(segment - 64) * 0x1p-4f
                              : 4.0f + static_cast<float>(segment - 112) * 0x1p-2f;
      float step = segment < 64 ? 0x1p-22f
                                : segment < 112 ? 0x1p-20f : 0x1p-18f;
