...
AccuracyStats measure(const Interval &interval) {
  ...
  for (uint32_t i = 0; i < kSamples; ++i) {
    ...
    float actual = SFUCore::compute(input, SFUOp::EXP2);
    float reference = static_cast<float>(std::exp2(static_cast<double>(input)));
...
int main() {
  const std::array<Interval, 4> intervals = {{{"[0.25, 0.5)", 0.25f, 0.5f},
    {"[0.5, 1)", 0.5f, 1.0f}, {"[1, 2)", 1.0f, 2.0f}, {"[2, 4)", 2.0f, 4.0f}}};
